...
Matrix NLBFunction::hessian(Matrix points){
  ...
  grad_a.push_back(
    -(2*(log(a)*(pow(log(b),2) + 1) + pow(log(a),3) + pow(log(a),2) - pow(log(b),2) - 1))/(pow(a,2)*pow(fun(points),2))
  );
  ...
  grad_b.push_back(
     -(2*(pow(log(a),2)*(log(b) - 1) + pow(log(b),3) + pow(log(b),2) + log(b) - 1))/(pow(b,2)*pow(fun(points),2))
  );
